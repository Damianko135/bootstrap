...
- `bootstrap.ps1` — single entrypoint. Supported `-Action` values:
  - `install` (default): remove bloatware, install packages, deploy Office, and write profile
...
- `uninstallList.json` — bloatware removal list used by the debloat step of `install` (unrelated to package management; there's no separate package-uninstall action)
...
**Option A** — opens an elevated window and runs the script directly (avoids the classic `iwr | iex` pattern that AV heuristics key on, but may still trigger AV on managed machines — this is a small, low-reputation repo, and no syntax tweak fixes that):

```powershell
Start-Process powershell -Verb RunAs -ArgumentList '-NoProfile', '-ExecutionPolicy', 'Bypass', '-Command', "& ([scriptblock]::Create((irm 'https://raw.githubusercontent.com/Damianko135/bootstrap/master/bootstrap.ps1')))"
```
...
- If you run the script without Administrator privileges and the selected action requires elevation (`install` or `office`), the script will attempt to relaunch itself elevated (using whichever PowerShell host you're already running) and prompt for UAC.
- There's no package-uninstall action. To remove something, uninstall it the normal way (Settings > Apps, or `choco uninstall <id>` / `winget uninstall --id <id>` directly).
